#!/bin/bash

VAL_DATA_DIR=/path/to/your/eval_data
RESULTS_BASE_DIR=/path/to/your/results_dir
MODEL_PATH=/path/to/your/base_model
CHECKPOINT_PATH=/path/to/your/checkpoint
GPUS=4

SKIP_FILE="val.parquet"

WANDB_PROJECT_NAME=MMDynOpt-eval

export CUDA_VISIBLE_DEVICES=0,1,2,3
export CUDA_DEVICE_ORDER=PCI_BUS_ID

export RAY_PORT=6662
export RAY_DASHBOARD_PORT=6676
export RAY_TEMP_DIR=/tmp/ray_eval_${RAY_PORT}

export NO_PROXY="localhost,127.0.0.1,0.0.0.0"
export no_proxy=$NO_PROXY

export NCCL_IB_DISABLE=1
export NCCL_SHM_DISABLE=0
export NCCL_ALGO=Ring
export NCCL_DEBUG=WARN
export TOKENIZERS_PARALLELISM=true

export WANDB_MODE=offline
export WANDB_DIR=./wandb_logs

export RAY_raylet_start_wait_time_s=300
export RAY_gcs_server_request_timeout_seconds=60

SPILL_DIR=${RAY_TEMP_DIR}/spill

cleanup_my_ports() {
    for PORT in ${RAY_PORT} ${RAY_DASHBOARD_PORT}; do
        PIDS=$(lsof -ti :${PORT} 2>/dev/null || true)
        if [ -n "$PIDS" ]; then
            echo "Cleaning up processes on port ${PORT}: ${PIDS}"
            kill -9 $PIDS 2>/dev/null || true
        fi
    done
    sleep 2
}

start_ray() {
    unset RAY_ADDRESS
    export RAY_ENABLE_WINDOWS_OR_OSX_CLUSTER=0
    rm -rf ${RAY_TEMP_DIR}
    mkdir -p ${RAY_TEMP_DIR} ${SPILL_DIR}
    SPILL_DIR=${RAY_TEMP_DIR}/spill
    mkdir -p ${SPILL_DIR}

    NODE_IP=$(hostname -i | awk '{print $1}')

    ray start --head \
        --node-ip-address=${NODE_IP} \
        --num-cpus=8 \
        --num-gpus=4 \
        --port=${RAY_PORT} \
        --include-dashboard=False \
        --temp-dir=${RAY_TEMP_DIR} \
        --object-spilling-directory=${SPILL_DIR} \
        --object-store-memory=8000000000

    export RAY_ADDRESS="${NODE_IP}:${RAY_PORT}"
    sleep 10
    ray status --address="${NODE_IP}:${RAY_PORT}" || { echo "Ray start failed, exiting"; exit 1; }
}

run_eval() {
    local PQ_FILE=$1
    local SAVE_DIR=$2
    local DATASET_NAME=$3

    python3 -m mmdynopt_agent.trainer.multimodal.main_ppo \
        algorithm.adv_estimator=grpo \
        data.train_files=$PQ_FILE \
        data.val_files=$PQ_FILE \
        data.train_batch_size=128 \
        data.max_prompt_length=4096 \
        data.max_response_length=4096 \
        data.image_key=images \
        data.user_prompt_round_1=mmdynopt_agent/prompts/initial_prompt.pkl \
        data.shuffle=False \
        actor_rollout_ref.model.path=$MODEL_PATH \
        actor_rollout_ref.actor.optim.lr=2e-6 \
        actor_rollout_ref.actor.optim.lr_sigmoid_decay_warmup=True \
        actor_rollout_ref.actor.optim.lr_sigmoid_decay_ratio=0.95 \
        actor_rollout_ref.actor.optim.lr_sigmoid_decay_warmup_steps=45 \
        actor_rollout_ref.model.use_remove_padding=True \
        actor_rollout_ref.actor.ppo_mini_batch_size=128 \
        actor_rollout_ref.actor.entropy_coeff=0 \
        actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=2 \
        actor_rollout_ref.actor.use_multi_turn_response_mask=True \
        actor_rollout_ref.actor.use_kl_loss=True \
        actor_rollout_ref.actor.kl_loss_coef=0.001 \
        actor_rollout_ref.actor.kl_loss_type=low_var_kl \
        actor_rollout_ref.model.enable_gradient_checkpointing=True \
        actor_rollout_ref.actor.fsdp_config.param_offload=False \
        actor_rollout_ref.actor.fsdp_config.optimizer_offload=False \
        actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=2 \
        actor_rollout_ref.rollout.tensor_model_parallel_size=1 \
        actor_rollout_ref.rollout.name=vllm_multiturn_mmdynopt \
        actor_rollout_ref.rollout.gpu_memory_utilization=0.5 \
        actor_rollout_ref.rollout.enable_chunked_prefill=False \
        actor_rollout_ref.rollout.enforce_eager=False \
        actor_rollout_ref.rollout.free_cache_engine=False \
        actor_rollout_ref.rollout.n=1 \
        actor_rollout_ref.rollout.max_gen_round=15 \
        actor_rollout_ref.rollout.response_length_total=8192 \
        actor_rollout_ref.rollout.max_model_len=16384 \
        actor_rollout_ref.rollout.max_num_batched_tokens=16384 \
        actor_rollout_ref.rollout.search.topk=5 \
        actor_rollout_ref.rollout.search.parallel_tool_call=True \
        actor_rollout_ref.rollout.search.parallel_tool_call_threads=8 \
        actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu=2 \
        actor_rollout_ref.ref.fsdp_config.param_offload=False \
        algorithm.kl_ctrl.kl_coef=0.001 \
        trainer.critic_warmup=0 \
        'trainer.logger=[wandb,console]' \
        trainer.project_name=$WANDB_PROJECT_NAME \
        trainer.experiment_name=eval-${DATASET_NAME} \
        trainer.n_gpus_per_node=$GPUS \
        trainer.nnodes=1 \
        trainer.save_freq=1000 \
        +trainer.max_actor_ckpt_to_keep=3 \
        +trainer.max_critic_ckpt_to_keep=3 \
        trainer.test_freq=1 \
        trainer.total_epochs=1 \
        +trainer.search_penalty=0.1 \
        +trainer.format_penalty=0.1 \
        +trainer.reward_mode="EM" \
        +trainer.val_before_train=True \
        +algorithm.filter_groups.enable=False \
        trainer.resume_mode=auto \
        trainer.val_only=$CHECKPOINT_PATH \
        trainer.val_only_save_dir=$SAVE_DIR
}

echo "============================================"
echo "Batch evaluation started"
echo "Data dir: ${VAL_DATA_DIR}"
echo "Results dir: ${RESULTS_BASE_DIR}"
echo "Checkpoint: ${CHECKPOINT_PATH}"
echo "Ray ports: ${RAY_PORT} / ${RAY_DASHBOARD_PORT}"
echo "============================================"

mkdir -p "${RESULTS_BASE_DIR}"

SUCCESS=0
FAIL=0
FAIL_LIST=""

for PQ_FILE in ${VAL_DATA_DIR}/*.parquet; do
    FILENAME=$(basename "$PQ_FILE")

    if [ "$FILENAME" = "$SKIP_FILE" ]; then
        echo "[SKIP] ${FILENAME}"
        continue
    fi

    DATASET_NAME="${FILENAME%.parquet}"
    SAVE_DIR="${RESULTS_BASE_DIR}/${DATASET_NAME}"

    if [ -f "${SAVE_DIR}/res.json" ]; then
        echo "[DONE, SKIP] ${DATASET_NAME}"
        SUCCESS=$((SUCCESS + 1))
        continue
    fi

    echo ""
    echo "==================== Start eval: ${DATASET_NAME} ===================="

    cleanup_my_ports
    start_ray

    set +e
    run_eval "$PQ_FILE" "$SAVE_DIR" "$DATASET_NAME"
    EXIT_CODE=$?
    set -e

    cleanup_my_ports

    if [ $EXIT_CODE -eq 0 ] && [ -f "${SAVE_DIR}/res.json" ]; then
        echo "==================== Done: ${DATASET_NAME} ===================="
        SUCCESS=$((SUCCESS + 1))
    else
        echo "==================== Failed: ${DATASET_NAME} (exit=${EXIT_CODE}) ===================="
        FAIL=$((FAIL + 1))
        FAIL_LIST="${FAIL_LIST} ${DATASET_NAME}"
    fi
done

cleanup_my_ports

echo ""
echo "============================================"
echo "Batch evaluation complete"
echo "Success: ${SUCCESS}"
echo "Failed: ${FAIL}"
if [ -n "$FAIL_LIST" ]; then
    echo "Failed list:${FAIL_LIST}"
fi
echo "Results dir: ${RESULTS_BASE_DIR}/"
echo "============================================"
