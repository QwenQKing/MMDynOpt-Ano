...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if role == 'actor':
            ...
            from mmdynopt_agent.utils.torch_functional import get_sigmoid_decay_schedule
            ...
            if lr_sigmoid_decay:
                lr_sigmoid_decay_ratio = optim_config.get('lr_sigmoid_decay_ratio', 0)
                num_warmup_steps = optim_config.get(
                    'lr_sigmoid_decay_warmup_steps', 60
                )
                print(f'[MMDynOpt LR Sigmoid Decay] {lr_sigmoid_decay_ratio=} {num_warmup_steps=}')
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
        ...
        elif self.config.rollout.name == 'vllm_multiturn_mmdynopt':
            from verl.workers.rollout.vllm_rollout import vllm_mode
            ...
            if self.config.rollout.use_fire_sampling:
                ...
                from mmdynopt_agent.workers.multimodal.rollout.vllm_rollout_spmd import vLLMRollout_MultiTurn_MMDynOpt
            from verl.workers.sharding_manager.fsdp_vllm import FSDPVLLMShardingManager

            log_gpu_memory_usage('Before building vllm_multi_turn rollout', logger=None)
            local_path = copy_to_local(self.config.model.path)
            if vllm_mode == 'spmd':
                rollout = vLLMRollout_MultiTurn_MMDynOpt(
                    model_path=local_path,
                    config=self.config.rollout,
                    tokenizer=self.tokenizer,
                    model_hf_config=self.actor_model_config,
    ...
    def init_model(self):
        from mmdynopt_agent.workers.multimodal.actor.dp_actor import MmDataParallelPPOActor
